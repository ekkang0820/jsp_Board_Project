...
<title>게시글</title>
 <style>      
    ...
    .navbar-brand {
        font-weight: bold;
        color: #8b5a2b; /* 네비게이션 바 브랜드 색상 변경 */
    }
    .footer {
        padding: 20px 0;
		...
		<div class="container">
			<div class="row">
				<table  class="table" style="text-align: center;border: 1px solid gray; background-color: white;">
					<thead>
						<tr>
							...
							<th style="background-color: #eeeeee; text-align: center;"><a href="reverseboard.jsp">작성일▲</a></th>							
						</tr>
					</thead>
					<%
						...
						{
							li = BoardDAO.getListtitle(pagenumber, keyword);
						}
						else if(searchoption.equals("txt"))
						{
							li = BoardDAO.getListtxt(pagenumber, keyword);
						}
						else if(searchoption.equals("reply"))
						{
							li = BoardDAO.getListreply(pagenumber, keyword);
						}
						else
						{
						li = BoardDAO.getList(pagenumber, keyword);
				...
				<%
					...
					{
				%>
				<a href="board.jsp?pagenumber=<%=pagenumber - 1%>&keyword=<%=keyword %>&searchoption=<%=searchoption %>" class="btn btn-primary">이전</a>
				<%
					}
				%>
				<%
					...
					{
						%>
						<a href="board.jsp?pagenumber=1&keyword=<%=keyword %>&searchoption=<%=searchoption %>" >1 | </a>
						<%
					}
				...
				%>
				<a href="board.jsp?pagenumber=<%=i-2  %>&keyword=<%=keyword %>&searchoption=<%=searchoption %>" ><%= i-2 %> | </a>
				<%
				...
				%>				
				 	<a href="board.jsp?pagenumber=<%=pagenumber-1 %>&keyword=<%=keyword %>&searchoption=<%=searchoption %>" ><%= pagenumber-1 %> | </a>
				<%
					}
					}
				%>
				<a href="board.jsp?pagenumber=<%=pagenumber%>&keyword=<%=keyword %>&searchoption=<%=searchoption %>" class="btn btn-primary"><%=pagenumber%></a>
				<%
					int nextpageindex = 3;
					int nextpageflag = 0;
					if(totalpage > 3 ){
				...
				%>
				<a href="board.jsp?pagenumber=<%=i+1  %>&keyword=<%=keyword %>&searchoption=<%=searchoption %>" > | <%= i+1 %></a>
				...
				%>
				<a href="board.jsp?pagenumber=2&keyword=<%=keyword %>&searchoption=<%=searchoption %>" > | 2</a>
				<%
					}
				if(BoardDAO.nextPage(pagenumber + 1 )&& totalpage != pagenumber){
				%>
			    
				<a href="board.jsp?pagenumber=<%=pagenumber + 1%>&keyword=<%=keyword %>&searchoption=<%=searchoption %>" class="btn btn-primary">다음</a>
				<%
